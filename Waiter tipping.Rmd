...
#In this output, 'total_bill' and 'size' appear to be statistically significant predictors of tip amounts, as indicated by their low p-values. The other predictor variables ('sex', 'smoker', 'day', 'time') do not appear to be statistically significant based on their p-values.
```
...
Finding the optima lambda for the Rigde and lasso with Cross Validation
```{r}
...
```
iii. LASSO regularization: Penalizes coefficients to prevent overfitting.


```{r}

# Perform ridge regression
lasso_model <- glmnet(x_train_matrix, y_train_vector, alpha = 1, lambda = 0.02)
...
For each of variable selection method, RIDGE and LASSO, use your
trained model to predict tip amounts for the testing set.
```{r}
...
ii. Calculate the test mean squared error (MSE) of each model.
```{r}
...
mse_lasso
```
iii. Comment on the results of each model (Full, Stepwise, RIDGE, and
LASSO) and select the best model based on MSE.
...
2. Interaction Term:
(a) Include an interaction term in your final best model from 2 and 3.
```{r}
...
```
(b) Prediction and MSE Calculation:
i. Use your trained model to predict tip amounts for the testing set.
...
ii. Calculate the test mean squared error (MSE) of the model.
```{r}
...
# Plot residuals against fitted values for linearity and homoscedasticity
plot(best_model_aic$fitted.values, residuals, 
...
```
5 Conclusion
1. Discuss the practical implications of your findings. For example, which factors
contribute most to higher tips?

Answer:


2.How does the interaction term impact tip amounts?
```{r}
